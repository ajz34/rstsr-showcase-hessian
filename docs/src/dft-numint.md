...
请留意对于 LDA, GGA 与 mGGA 的 $\bm{\xi}$ 并不一致。LDA 只有第一个分量；GGA 只有前四个分量；mGGA 则包含全部五个分量。

DFT 基本参量 $\bm{\xi}$ 的分量角标记为 $\chi$。
...
| 变量名 | 公式表达 | 维度 (PySCF/row-major) | 维度 (REST/col-major) |
|--|--|--|--|
...
请留意，作为 row-major 与 col-major 不同的程序，PySCF 中的 $\phi_{g \mu}^{*}$ 尽管表面维度是 $(*, g, \mu)$，但其内存中的连续性是 $g$ 即格点索引在最内层，其次 $\mu$ 即基函数，最后是原子轨道导数分量；这个内存连续性顺序与 REST 实际上是相同的。对于其他分量，REST 与 PySCF 的维度相反，内存连续性相同。
...
请留意，`fxc` 张量是具有 $(\sigma, \sigma')$ 与 $(\chi, \chi')$ 对称性的。
