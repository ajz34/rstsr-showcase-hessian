...
class TestHessianRKS(unittest.TestCase):
    ...
    def test_make_hessian_setup_whole(self):
        ...
        self.assertAlmostEqual(lib.fp(result["de_vxc_diag"]), 52.680615293622338, places=5)
        self.assertAlmostEqual(lib.fp(result["de_vxc_off"]), -33.602789997689506, places=5)
        self.assertAlmostEqual(lib.fp(result["de_fxc"]), -20.132874762943889, places=5)
        self.assertAlmostEqual(lib.fp(result["vmat_deriv1"]), -4.579019717395766, places=5)
    ...
    def test_make_hess(self):
        ...
        Mirrors `test_make_hess` in `test_hessian_rks_svwn.py`, but the XC
        ...
        (~5e-4 for LDA, smaller than the GGA/MGGA cases).
        """
        natm = mol.natm
        ...
        self.assertTrue(
            np.allclose(de_hess, ref_value["de_ref"], atol=1e-3, rtol=0),
