...
def setUpModule():
    ...
    # 0.1*HF admixture to avoid the PySCF with_k=False UKS df-Hessian bug
    # (same workaround as the reference notebook 07-2 and the grid-fixed test).
...
class TestHessianUKS(unittest.TestCase):
    ...
    def test_make_hessian_setup_whole(self):
        ...
        self.assertAlmostEqual(lib.fp(result["de_fxc"]), -19.718052236087, places=5)
    ...
    def test_make_hess(self):
        ...
        Mirrors `test_make_hess` in `test_hessian_uks_svwn.py`, but the XC
        ...
        (~6e-4 for the LDA case, the smallest of the three xc families).
        """
        natm = mol.natm
        ni = mf._numint
