...
def setUpModule():
    ...
    PATH_REF = PATH_PROTOTYPE + "nh3_u_tpss0_decomp.npz"

    mol = gto.Mole(atom=xyz, basis="def2-TZVP", spin=2, max_memory=8000).build()
    mf = dft.UKS(mol, xc="TPSS0").density_fit()
...
class TestHessianUKS(unittest.TestCase):
    ...
    def test_make_hessian_setup_whole(self):
        ...
        self.assertAlmostEqual(lib.fp(result["de_fxc"]), -28.913589693130, places=5)
    ...
    def test_make_hess(self):
        ...
        Mirrors `test_make_hess` in `test_hessian_uks_tpss0.py`, but the XC
        ...
        self.assertTrue(
            np.allclose(de_hess, ref_value["de_ref"], atol=5e-3, rtol=0),
