...
impl<'a> NIMatmul<'a> {
    ...
    pub fn make_rho_from_dm(&mut self, dm_list: &[TsrView], den_type: NIDenType) -> Tsr {
        ...
        let nao = ao.shape()[1];
        for dm in dm_list {
            check_shape!(dm.ndim(), 2, "Each density matrix must be 2-dim");
            check_shape!(dm.shape()[0..2], [nao, nao], "Density matrix must match AO dimension");
        }
    ...
    pub fn make_rho_from_homogeneous_braket(
        ...
        let nao = ao.shape()[1];
        for bra in bra_list {
            check_shape!(bra.ndim(), 2, "Each braket must be 2-dim");
            check_shape!(bra.shape()[0], nao, "Bra's first dimension must match AO dimension");
        }
    ...
    pub fn make_rho_from_one_bra_mult_ket(
        ...
        check_shape!(bra.shape()[0], nao, "Bra first dimension must match AO dimension");
        let nocc = bra.shape()[1];
        for ket in ket_list {
            check_shape!(ket.ndim(), 2, "Each ket must be 2-dim");
            check_shape!(ket.shape()[0], nao, "Ket first dimension must match AO dimension");
            check_shape!(ket.shape()[1], nocc, "Ket second dimension must match bra");
        }
    ...
    pub fn make_rho_from_mult_bra_mult_ket(
        ...
        for (bra, ket) in bra_list.iter().zip(ket_list.iter()) {
            check_shape!(bra.ndim(), 2, "Each bra must be 2-dim");
            check_shape!(ket.ndim(), 2, "Each ket must be 2-dim");
            check_shape!(nao, bra.shape()[0], "Bra first dimension must match AO dimension");
            check_shape!(nao, ket.shape()[0], "Ket first dimension must match AO dimension");
            check_shape!(bra.shape()[1], ket.shape()[1], "Bra and ket occupation must match");
        }
        let nset = bra_list.len();
