...
pub fn get_rho_from_dm_with_output(
    ...
    check_shape!(out.shape().clone(), [ngrids, nvar, nset], "Output shape mismatch");
...
pub fn get_rho_from_homogeneous_braket_with_output(
    ...
    for bra in bra_list {
        ...
        check_shape!(nao, bra.shape()[0], "AO dimension must match braket dimension");
    }
...
pub fn get_rho_from_mult_bra_mult_ket_with_output(
    ...
    check_shape!(bra_list.len(), ket_list.len(), "bra_list and ket_list must have same length");
    let nocc_max = bra_list.iter().map(|bra| bra.shape()[1]).max().unwrap_or(0);
