...
fn contract_ao_wv_without_symmetrize(
    ...
    check_shape!(den_type.num_nvar(), nvar, "Dimension mismatch for input density type");
    check_shape!(ao.ndim(), 3, "AO tensor must be 3-dim");
    let nao = ao.shape()[1];
    check_shape!(ao.shape()[0], ngrids, "AO grids dimension mismatch");
    check_shape!(ao.shape()[2] >= den_type.num_ao_comp(), "AO component dimension insufficient");

    check_shape!(out.shape(), [nao, nao], "Output shape mismatch");
    check_shape!(buf.len() >= ngrids * nao, "Buffer length insufficient");
...
pub fn rks_vxc_pot_with_eff_with_output(
    ...
    check_shape!(weights.shape(), [ngrids], "Weights shape mismatch");
    check_shape!(den_type.num_nvar(), nvar, "Dimension mismatch for input density type");
    check_shape!(ao.ndim(), 3, "AO tensor must be 3-dim");
    check_shape!(ao.shape()[0], ngrids, "AO grids dimension mismatch");
    check_shape!(ao.shape()[2] >= den_type.num_ao_comp(), "AO component dimension insufficient");
    let nao = ao.shape()[1];
    check_shape!(vxc.shape(), [nao, nao], "Output shape mismatch");
...
pub fn rks_fxc_pot_with_eff_with_output(
    ...
    check_shape!(fxc_eff.shape(), [ngrids, nvar, nvar], "fxc_eff shape mismatch");
    check_shape!(weights.shape(), [ngrids], "Weights shape mismatch");
    check_shape!(den_type.num_nvar(), nvar, "Dimension mismatch for input density type");
    check_shape!(ao.ndim(), 3, "AO tensor must be 3-dim");
    check_shape!(ao.shape()[0], ngrids, "AO grids dimension mismatch");
    check_shape!(ao.shape()[2] >= den_type.num_ao_comp(), "AO component dimension insufficient");
    let nao = ao.shape()[1];
    check_shape!(fxc.shape(), [nao, nao, nset], "Output shape mismatch");
...
fn contract_ao_wv_bra(
    ...
    check_shape!(ao.shape()[0], ngrids, "AO grids dimension mismatch");
    check_shape!(ao.shape()[2] >= den_type.num_ao_comp(), "AO component dimension insufficient");
    check_shape!(ao_bra.ndim(), 3, "ao_bra tensor must be 3-dim");
    check_shape!(ao_bra.shape()[0], ngrids, "ao_bra grids dimension mismatch");
    let nocc = ao_bra.shape()[1];
    check_shape!(ao_bra.shape()[2] >= den_type.num_ao_comp(), "ao_bra component dimension insufficient");
    check_shape!(out.shape(), [nao, nocc], "Output shape mismatch");
    check_shape!(buf.len() >= ngrids * nocc, "Buffer length insufficient");
...
pub fn rks_fxc_pot_with_eff_bra_trans_with_output(
    ...
    check_shape!(ao.shape()[2] >= den_type.num_ao_comp(), "AO component dimension insufficient");
    let nao = ao.shape()[1];
    check_shape!(bra.ndim(), 2, "bra must be 2-dim");
    check_shape!(bra.shape()[0], nao, "bra first dimension must match nao");
    let nocc = bra.shape()[1];
    check_shape!(fxc.shape(), [nao, nocc, nset], "Output shape mismatch");
...
pub fn rks_kxc_pot_with_eff_with_output(
    ...
    let nvar = rho1.shape()[1];
    let ngrids = rho1.shape()[0];
    check_shape!(kxc_eff.shape(), [ngrids, nvar, nvar, nvar], "kxc_eff shape mismatch");
    check_shape!(rho2.shape()[0..2], [ngrids, nvar], "rho2 shape mismatch");
    check_shape!(weights.shape(), [ngrids], "Weights shape mismatch");
    check_shape!(den_type.num_nvar(), nvar, "Dimension mismatch for input density type");
    check_shape!(ao.ndim(), 3, "AO tensor must be 3-dim");
    check_shape!(ao.shape()[0], ngrids, "AO grids dimension mismatch");
    check_shape!(ao.shape()[2] >= den_type.num_ao_comp(), "AO component dimension insufficient");
    let nao = ao.shape()[1];
    check_shape!(kxc.shape(), [nao, nao, nset1, nset2], "Output shape mismatch");
...
pub fn uks_vxc_pot_with_eff_with_output(
    ...
    let nvar = vxc_eff.shape()[1];
    let ngrids = vxc_eff.shape()[0];
    check_shape!(vxc_eff.shape()[2], 2, "vxc_eff must have 2 spin channels");
    check_shape!(weights.shape(), [ngrids], "Weights shape mismatch");
    check_shape!(den_type.num_nvar(), nvar, "Dimension mismatch for input density type");
    check_shape!(ao.ndim(), 3, "AO tensor must be 3-dim");
    check_shape!(ao.shape()[0], ngrids, "AO grids dimension mismatch");
    check_shape!(ao.shape()[2] >= den_type.num_ao_comp(), "AO component dimension insufficient");
    let nao = ao.shape()[1];
    check_shape!(vxc.shape(), [nao, nao, 2], "Output shape mismatch");
...
pub fn uks_fxc_pot_with_eff_with_output(
    ...
    let nvar = rho1.shape()[1];
    let ngrids = rho1.shape()[0];
    check_shape!(rho1.shape()[2], 2, "rho1 must have 2 spin channels");
    check_shape!(fxc_eff.shape(), [ngrids, nvar, 2, nvar, 2], "fxc_eff shape mismatch");
    check_shape!(weights.shape(), [ngrids], "Weights shape mismatch");
    check_shape!(den_type.num_nvar(), nvar, "Dimension mismatch for input density type");
    check_shape!(ao.ndim(), 3, "AO tensor must be 3-dim");
    check_shape!(ao.shape()[0], ngrids, "AO grids dimension mismatch");
    check_shape!(ao.shape()[2] >= den_type.num_ao_comp(), "AO component dimension insufficient");
    let nao = ao.shape()[1];
    check_shape!(fxc.shape(), [nao, nao, 2, nset], "Output shape mismatch");
...
pub fn uks_kxc_pot_with_eff_with_output(
    ...
    let ngrids = rho1.shape()[0];
    check_shape!(rho1.shape()[2], 2, "rho1 must have 2 spin channels");
    check_shape!(rho2.shape()[0..3], [ngrids, nvar, 2], "rho2 shape mismatch");
    check_shape!(kxc_eff.shape(), [ngrids, nvar, 2, nvar, 2, nvar, 2], "kxc_eff shape mismatch");
    check_shape!(weights.shape(), [ngrids], "Weights shape mismatch");
    check_shape!(den_type.num_nvar(), nvar, "Dimension mismatch for input density type");
    check_shape!(ao.ndim(), 3, "AO tensor must be 3-dim");
    check_shape!(ao.shape()[0], ngrids, "AO grids dimension mismatch");
    check_shape!(ao.shape()[2] >= den_type.num_ao_comp(), "AO component dimension insufficient");
    let nao = ao.shape()[1];
    check_shape!(kxc.shape(), [nao, nao, 2, nset1, nset2], "Output shape mismatch");
...
pub fn uks_fxc_pot_with_eff_bra_trans_with_output(
    ...
    for bra_spin in bra.iter() {
        check_shape!(bra_spin.ndim(), 2, "bra tensors must be 2-dim");
        check_shape!(bra_spin.shape()[0], nao, "bra first dimension must match nao");
    }
    let nocc_alpha = bra[0].shape()[1];
    let nocc_beta = bra[1].shape()[1];
    let nocc_max = nocc_alpha.max(nocc_beta);
    check_shape!(fxc[0].shape(), [nao, nocc_alpha, nset], "Output shape mismatch");
    check_shape!(fxc[1].shape(), [nao, nocc_beta, nset], "Output shape mismatch");
