...
pub fn libxc_eval_inner(xc_func: &LibXCFunctional, rho: TsrView, deriv: usize) -> (Vec<f64>, LibXCOutputLayout) {
    ...
    match xc_func.spin() {
        Unpolarized => check_shape!(rho.ndim(), 2, "rho for unpolarized functionals must be a 2-dim tensor"),
        Polarized => {
            check_shape!(rho.ndim(), 3, "rho for polarized functionals must be a 3-dim tensor");
            check_shape!(rho.shape()[2], 2, "rho for polarized functionals must have last dimension of size 2");
        },
    }
    ...
    check_shape!(rho.shape()[1] >= den_type.num_nvar(), "Input density does not have enough components");
