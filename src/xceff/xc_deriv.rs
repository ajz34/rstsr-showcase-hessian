...
pub fn transform_xc_inner(rho: TsrView, xc_val: TsrView, den_type: XCDenType, spin: LibXCSpin, order: usize) -> Tsr {
    ...
    // check dimensions
    check_shape!(xc_val.shape()[0], ngrids, "xc_val length (grids) mismatch");
    check_shape!(xc_val.ndim(), 2, "xc_val must be a 2-dim tensor");
    // check shape [ngrids, nvar, nspin if exist], otherwise panic
    match spin {
        Unpolarized => check_shape!(rho.ndim(), 2, "rho must be a 2-dim tensor"),
        Polarized => {
            check_shape!(rho.ndim(), 3, "rho must be a 3-dim tensor");
            check_shape!(rho.shape()[2], 2, "rho last dimension should be 2 for polarized case");
        },
    };
    check_shape!(rho.shape()[0], ngrids, "rho first dimension must be grids");
    check_shape!(rho.shape()[1] >= nvar, "rho second dimension (variables) should be larger than {nvar}");
    ...
    check_shape!(xc_val.shape()[1] >= *offset_max, "xc_val length should be larger than {offset_max}");
