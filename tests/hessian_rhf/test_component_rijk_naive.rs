...
fn test_hess_ri_jk_skeleton_naive(hess_case: &CaseAmoniaRHF) {
    ...
    // test ri-j
    let de_dict = get_decomposed_rij_skeleton_deriv2_naive(mol, aux, mo_coeff.view(), mo_occ.view(), None);
    ...
    // test ri-k
    let de_dict = get_decomposed_rik_skeleton_deriv2_naive(mol, aux, mo_coeff.view(), mo_occ.view(), None);
    for (key, de) in de_dict {
        let de_ref = ref_dict[key].to_owned().into_reverse_axes();
        assert!(rt::allclose(de.view(), de_ref.view(), (1e-4, 1e-6)));
    }
...
fn test_hess_ri_jk_deriv1_ao_naive(hess_case: &CaseAmoniaRHF) {
    ...
    let de_dict = get_rij_deriv1_ao_naive(mol, aux, mo_coeff.view(), mo_occ.view(), None);
    ...
    let de_dict = get_rik_deriv1_ao_naive(mol, aux, mo_coeff.view(), mo_occ.view(), None);
    // the deriv1 ao here is [u, v, t, A], while for python it is [A, t, u, v]; swap first two axes
