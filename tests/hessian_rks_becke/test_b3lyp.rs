...
#[rstest]
fn test_setup(hess_case_b3lyp: &CaseAmoniaRKSBecke) {
...
#[rstest]
fn test_make_hess(hess_case_b3lyp: &CaseAmoniaRKSBecke) {
    let CaseAmoniaRKSBecke {
        mol,
        aux,
        mo_coeff,
    ...
    // vs the (grid-fixed) PySCF reference: grid-shift magnitude for GGA
    ...
    assert!(maxdiff < 2e-3, "max abs diff = {maxdiff}");

    // full Hessian translational invariance (< 1e-7)
    let inv = de_hess.view().sum_axes([-1, -2]);
    let m = inv.abs().max();
