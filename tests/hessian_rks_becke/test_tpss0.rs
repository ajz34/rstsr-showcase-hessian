...
#[rstest]
fn test_setup(hess_case_tpss0: &CaseAmoniaRKSBecke) {
...
#[rstest]
fn test_make_hess(hess_case_tpss0: &CaseAmoniaRKSBecke) {
    let CaseAmoniaRKSBecke {
        mol,
        aux,
        mo_coeff,
    ...
    let mut rijk_obj = RHessRIJK::new_without_cderi(&mol, &aux, 1.0, 0.25);
    ...
    // vs the (grid-fixed) PySCF reference: grid-shift magnitude for MGGA
