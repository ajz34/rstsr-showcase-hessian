...
#[rstest]
fn test_whole(hess_case_tpss0: &CaseAmoniaUKS) {
    let CaseAmoniaUKS { mol, mo_coeff, mo_occ, grid_coords, grid_weights, ref_dict, .. } = hess_case_tpss0.clone();
    ...
    assert_abs_diff_eq!(fp(result["de_fxc"].view()), -28.913589693130, epsilon = 1e-4);
}

#[rstest]
fn test_make_hess(hess_case_tpss0: &CaseAmoniaUKS) {
    ...
    assert_abs_diff_eq!(fp(de_hess.view()), 0.574483693293, epsilon = 5e-3);
